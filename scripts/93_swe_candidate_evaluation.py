#!/usr/bin/env python3
"""Private fixed-slot SWE host grading, gated after two blinded mechanism reviews."""
import argparse
import ast
...
OUT=COHORT/'private/candidate-evaluation'
...
def prepare():
    ...
    assert len(jobs)==len(patches)==48
    assert len({r['key'] for r in jobs})==48
    ...
    frozen=[]
    sources=[Path(__file__).resolve(),HELPER,b.PARSER,COHORT/'protocol.json',COHORT/'selection.json',
    ...
    for job,row in zip(jobs,patches):
        ...
        item=dict(key=key,instance_id=iid,sample=job['sample'],base_commit=task['base_commit'],
                  stage='ungenerated' if not (GEN/'responses'/(key+'.json')).exists() else 'invalid_generation',
    ...
    protocol=dict(slots=frozen,slot_count=48,tasks=parent['tasks'],source_sha256={str(p):sha(p) for p in sources},
        ...
        authorization='Separate root JSON pinned to this protocol and at least2 distinct blinded-review hashes is mandatory before candidate imports/tests.',
    ...
    print(json.dumps(dict(slots=48,ready=sum(r['stage']=='ready' for r in frozen),review_flags=sum(bool(r['suspicious_additions']) for r in frozen),protocol_sha256=sha(OUT/'protocol.json'))),flush=True)
...
def run(authorization):
    ...
    reviews=auth['blinded_review_sha256'];assert len({str((ROOT/f).resolve()) for f in reviews})>=2
    ...
    for slot in p['slots']:
        ...
        print(json.dumps(dict(completed=len(results),slots=48,key=key,status=row['execution_status'])),flush=True)
...
if __name__=='__main__':
    ap=argparse.ArgumentParser();ap.add_argument('stage',choices=['prepare','run']);ap.add_argument('--authorization',type=Path);args=ap.parse_args()
