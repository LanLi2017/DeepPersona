...
OUT = COHORT.parent/'swe-debugging-trajectories-v2'
...
def prepare():
    ...
    OUT.mkdir()
    sources = [Path(__file__).resolve(), ROOT/'scripts/101_swe_debugging_replay_v2.py', ROOT/'scripts/92_swe_fresh_generation.py', ROOT/'scripts/93_swe_candidate_evaluation.py', ROOT/'scripts/85_swe_execution_bridge.py', COHORT/'selection.json', COHORT/'generation_context/manifest.json']
    jobs = []
    for iid in tasks:
    ...
    save(OUT/'protocol.json',dict(utc=utc(), model=MODEL, seed=None, sampling='API defaults, no seed or temperature override; independent histories',
        scope='Transport v2 after v1 HTTP400; same first two metadata tasks and two episodes each; feasibility only, no diversity intervention or learning effect',
        amendment='Omit None fields when replaying typed response output; retain full raw responses. Log structured API error fields. V1 remains frozen with one partial episode, three undispatched cancellations and all request charges/reservations.',
...
def step(key):
    ...
    with lock():
        ...
        try:
            ...
            state['turns']=turn
            state['history']+=response.model_dump(exclude_none=True)['output']
            ...
            if isinstance(body,dict):
                ...
                if isinstance(body,dict):record['api_error']={k:str(body[k])[:1000] for k in ['message','type','code','param'] if k in body}
