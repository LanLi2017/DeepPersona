#!/usr/bin/env python3
"""Second frozen comparison; first fresh suite is now development for normalization."""
...
g = c.g
OUT = c.OUT / "normalized-round2"
METHODS = {**c.FILES, "normalized": "swe_position_normalized.py"}


def prepare():
    OUT.mkdir(exist_ok=True)
    if (OUT / "protocol.json").exists():
        raise FileExistsError("Round 2 already frozen")
    files = [
        g.OUT / "records.json",
    ...
    g.s.write_json(
        OUT / "protocol.json",
        {
            ...
            "scope": "Original 26 pairs, 4 old probes, and first 18 fresh probes are development for normalized method. A separately authored second suite is opened only after its candidate is frozen. No benchmark outcomes or human gold.",
    ...
    print("Second candidate, input suite hashes, and rules frozen.")
