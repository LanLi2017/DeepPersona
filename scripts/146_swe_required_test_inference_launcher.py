#!/usr/bin/env python3
"""Required-test endpoint namespace; unchanged136 external deadline and cleanup logic."""
...
OUT = ROOT / "runs/swe-diversity-selection/swe-localize-repair-support/autonomous_inference_required_tests"
...
def main():
    ...
    assert auth["inference_script_sha256"] == sha(ROOT / "scripts/144_swe_required_test_inference.py")
    ...
    cmd = [str(ROOT / ".venv/bin/python"), str(ROOT / "scripts/144_swe_required_test_inference.py"), "worker"]
    save(
    ...
    subprocess.run(
        [str(ROOT / ".venv/bin/python"), str(ROOT / "scripts/144_swe_required_test_inference.py"), "collect"],
        cwd=ROOT,
        check=True,
    )
