...
OUT=BASE/'swe-sympy-retrieval-policies'
...
def main():
    ...
    outputs=[]
    inputs={str(p):sha(p) for p in [Path(__file__),old_path,OUT/'independent_policy_score_audit.json',OUT/'score_summary.json',OUT/'build_manifest.json',OUT/'completed_manifest.json']}
    for policy in summary['policies']:
        ...
        for score in policy['tasks']:
            ...
            accepted=[e['path'] for e in c['retrieval']['selection_events'] if e.get('kind')=='full_file']
            rows.append(dict(instance_id=iid,category=category,prequalified=score['prequalified'],missing_changed_files=missing_files,missing_deletions=missing_deletions,missing_insertion_boundaries=missing_boundaries,missing_anchors=missing_anchors,locality_and_completion_fit=locality and r['completion_fits'] is True,full_anchor_visible=full,accepted_whole_files=accepted,prompt_tokens=c['prompt_tokens']))
        passing={r['instance_id'] for r in rows if r['prequalified']}
        outputs.append(dict(policy=policy['policy'],categories=dict(Counter(r['category'] for r in rows)),prequalified=len(passing),gained_vs_baseline=sorted(passing-baseline),lost_vs_baseline=sorted(baseline-passing),locality_and_completion_fit=sum(r['locality_and_completion_fit'] for r in rows),accepted_whole_file_events=sum(len(r['accepted_whole_files']) for r in rows),tasks=rows))
