#!/usr/bin/env python3
"""Exploratory identical-multiset two-step SGD regrouping, FP32 MATH SDPA."""
import argparse
...
OUT=ROOT/'code-regrouping-interaction'
...
def prepare():
    ...
    labels={f'{r["task_id"]}:{r["sample"]}':r['correct'] for d in [POOL,EXPANSION] for r in read(d/'grades.jsonl')}
    by=defaultdict(list)
    ...
    sources=[INITIAL,OUT/'groups.json',OUT/'selected_raw.jsonl',OUT/'reference_tokens.jsonl']+[d/f for d in [POOL,EXPANSION] for f in ['protocol.json','raw.jsonl','grades.jsonl']]
    protocol={'seed':SEED,'scope':'Exploratory exact-multiset regrouping after prior method-development outcomes; not confirmation, functional gain, or novelty claim.',
              ...
              'task_ids':[r['task_id'] for r in tasks],'selection':'All22eligible tasks; four distinct recordIDs chosen by SHA(seed:regroup-choice:task:id), independent of metrics. Duplicate contents retained.',
              ...
              'finite_difference':{'epsilons':[.01,.003],'relative_gate':.05,'absolute_gate':1e-6,'probe':'Hd*d plus firstcorrect-trajectory Hessian times fixedseed Gaussian direction scaledL2 .01; probe alwayscomputed, not afeature.'},
    ...
    print(json.dumps({'tasks':22,'smoke_task':tasks[0]['task_id'],'selected_records':len(selected),'duplicate_content_tasks':sum(bool(t['same_token_content_pairs']) for t in tasks)}),flush=True)
...
def run(smoke,approved):
    ...
    groups=json.loads((OUT/'groups.json').read_text())[:1 if smoke else 22]
