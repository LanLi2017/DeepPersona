#!/usr/bin/env python3
"""External deadline for the127 constructed outline/read/repair learner smoke."""
...
OUT = ROOT / "runs/swe-diversity-selection/swe-localize-repair-support/learner_smoke"
...
def main():
    ...
    assert auth["smoke_script_sha256"] == sha(ROOT / "scripts/127_swe_localize_repair_smoke.py")
    ...
    env.update(
        ...
        SWE_SMOKE_EXTERNAL_TIMEOUT="3595+5",
        NVIDIA_TF32_OVERRIDE="0",
        HF_HUB_OFFLINE="1",
        TRANSFORMERS_OFFLINE="1",
        PYTHONHASHSEED="0",
    )
    cmd = [str(ROOT / ".venv/bin/python"), str(ROOT / "scripts/127_swe_localize_repair_smoke.py"), "run"]
    save(
    ...
    try:
        with (OUT / "outer_run.log").open("x") as log:
            ...
            try:
                code = proc.wait(timeout=max(0.0, start + 3595 - time.monotonic()))
                ...
                try:
                    ...
                    pass
                try:
                    code = proc.wait(timeout=max(0.0, start + 3600 - time.monotonic()))
                except subprocess.TimeoutExpired:
                    try:
    ...
    assert code == 0 and not timed_out and error is None, (
        "Technical smoke failed; preserve artifacts, no retry/fallback"
    )
