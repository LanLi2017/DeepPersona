#!/usr/bin/env python3
"""Conditional source-only runtime reuse; no assets/models read at import time."""
...
SMOKE = CAPACITY / "learner_smoke"
OUT = CAPACITY / "source_runtime"
...
def freeze():
    ...
    capacity = read(CAPACITY / "score_summary.json")
    cap = capacity["smallest_cap_meeting_source_gate"]
    assert cap in [12288, 24576] and capacity["minimum_required"] == 16
    assert capacity["technical_failures"] == 0 and capacity["quality_tests_run"] == 0
    assert capacity["evaluation_new_contexts"] == 0 and capacity["evaluation_private_values_read"] is False
    assert cap == min(r["cap"] for r in capacity["caps"] if r["prequalified"] >= 16)
    scored = next(r for r in capacity["caps"] if r["cap"] == cap)
    assert scored["assigned_sources"] == 32 and scored["prequalified"] >= 16
    ...
    cap_protocol = read(CAPACITY / "protocol.json")
    assert cap_protocol["source_tasks"] == sources
    ...
    assert outer["exit_code"] == 0 and outer["timed_out"] is False
    ...
    maps = [
        smoke_nonmodel_inputs,
        read(CAPACITY / "completed_manifest.json"),
        read(CAPACITY / "build_manifest.json")["artifact_sha256"],
        cap_protocol["source_sha256"],
        read(CAPACITY / "score_protocol.json")["inputs_sha256"],
        read(SMOKE / "run/completed_manifest.json"),
    ]
    for mapping in maps:
        verify(mapping)
    ...
    paths = [
        ...
        ROOT / "scripts/114_swe_sympy_context_capacity.py",
    ...
    save(
        ...
        dict(
            ...
            admission="Only capacity-prequalified SOURCE IDs acquire assets. No replacement or evaluation admission.",
    ...
    print(
        json.dumps(dict(frozen=True, selected_prompt_cap=cap, active_sources=len(ids), retained_sources=32)), flush=True
    )
